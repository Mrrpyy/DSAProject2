...
TrieNode* Trie::findChild(TrieNode* node, unsigned char character) {
...
const TrieNode* Trie::findChild(const TrieNode* node, unsigned char character) {
...
TrieNode* Trie::getOrCreateChild(TrieNode* node, unsigned char character) {
    ...
    node->children = new TrieChild(character, newNode, node->children);
    return newNode;
}
...
std::vector<Movie> Trie::searchAll(const std::string& title) const {
